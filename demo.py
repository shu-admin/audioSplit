...
class RequestApi(object):
    ...
    def all_api_request(self):
        ...
        self.result = self.get_result_request(taskid=taskid)['data']


# 注意：如果出现requests模块报错："NoneType" object has no attribute 'read', 请尝试将requests模块更新到2.20.0或以上版本(本demo测试版本为2.20.0)
# 输入讯飞开放平台的appid，secret_key和待转写的文件路径
if __name__ == '__main__':
    api = RequestApi(appid="9a541dd1", secret_key="720dcd48785a5b6987083cf6f6846bef", upload_file_path=r"./test-cn.mp3")
    api.all_api_request()
    content = api.result
    # print(api.result)
    ...
    for line in range(len(content)):
        with open(fileName,'a',encoding='utf-8') as f:
            f.write('wav'+'\\'+str(line+1)+'.wav '+str(resultJSON[line]['onebest'])+'\n')
        f.close()
